...
function buildIsMutatingFunction(ignoredMethods, useLodashFunctionImports) {
  const matchesSpecs = _.flow(
    _.map((fn) =>
      useLodashFunctionImports && isLodashFn(fn) ? fn.slice(2) : fn
    ),
...
function isAllowedFirstArgument(arg, node, allowFunctionProps) {
  return (
    ...
    isScopedVariable(arg, node.parent, allowFunctionProps)
  );
}

const create = function (context) {
  const ignoredMethods = _.getOr([], ['options', 0, 'ignoreMethods'], context);
  const useLodashFunctionImports = _.getOr(
    false,
    ['options', 0, 'useLodashFunctionImports'],
    context
  );
  const allowFunctionProps = _.getOr(
    false,
    ['options', 0, 'functionProps'],
    context
  );
  ...
  const isMutatingFunction = buildIsMutatingFunction(
    ignoredMethods,
  ...
  return {
    CallExpression(node) {
      if (
        isMutatingFunction(node.callee) &&
        !isAllowedFirstArgument(node.arguments[0], node, allowFunctionProps) &&
...
module.exports = {
  ...
  meta: {
    ...
    docs: {
      description:
        'Forbid the use of [`Object.assign()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/assign) with a variable as first argument.',
      recommended: 'error',
    },
  },
};
